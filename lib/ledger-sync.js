...
export function buildDefinitionsPushPayload(accounts, categories) {
  return {
    accounts: accounts.map((account) => ({
      id: toRemoteEntityId(account.remoteId),
 ...
 * @param {{ accountIds: Map<string, string>, categoryIds: Map<string, string> }} references
 */
export function buildTransactionsPushPayload(transactions, references) {
  return {
    transactions: transactions.map((transaction) => {
      ...
      return {
        id: toRemoteEntityId(transaction.remoteId),
