...
export function buildSnapshotFromRemotePayload(payload, options) {
  ...
  const remoteCategories = payload?.data?.categories ?? [];
  ...
  const categories = remoteCategories.map((category) => {
    ...
    return {
      ...
      isActive: !toBooleanFlag(category.is_deleted),
      isCustom: true,
      updatedAt: String(category.updatedAt ?? category.updated_at ?? options.fallbackSyncUpdatedAt),
      deletedAt: toBooleanFlag(category.is_deleted)
        ? String(category.deleted_at ?? category.updatedAt ?? category.updated_at ?? options.fallbackSyncUpdatedAt)
        : null,
    };
  });
